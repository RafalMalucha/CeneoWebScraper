...
class Product():
    ...
    def __str__(self) -> str:
        ...
        <br> opinions_count: {self.opinions_count}
        <br> pros_count: {self.pros_count} <br> cons_count: {self.cons_count}
        <br> average_score: {self.average_score}
        <br> opinions: <br>''' + '<br>'.join(str(opinion) for opinion in self.opinions)
    ...
    def export_product(self):
        ...
        with open(f'app/product/{self.product_id}.json', 'w', encoding='UTF-8') as file:
            json.dump(self.stats_to_dict(), file, indent=4, ensure_ascii=False)

    def import_product(self):
        if not os.path.exists(f'app/products/{self.procuct_id}.json'):
            with open(f'app/products/{self.product_id}.json', 'r', encoding='UTF-8') as jf:
                product = json.load(jf)
                self.procuct_id = product['product_id']
                self.procuct_name = product['product_name']
